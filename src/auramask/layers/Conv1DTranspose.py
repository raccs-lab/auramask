...
class Conv1DTranspose(layers.Conv2DTranspose):
    ...
    def call(self, inputs):
        ...
        outputs = ops.squeeze(outputs, axis=1)
    ...
    @property
    def input_spec(self):
        return layers.InputSpec(ndim=3, axes={self.channel_axis: self._input_dim})
...
def expand_input_shape(input_shape):
    ...
    return ops.shape(input_shape)


def squeeze_output_shape(output_shape):
    if len(output_shape) != 4:
        raise ValueError(
            "Rank of `output_shape` must be 4, got {}.".format(output_shape)
        )

    batch_size, _, length, channels = output_shape
    output_shape = ops.shape([batch_size, length, channels])
    return ops.shape(output_shape)
